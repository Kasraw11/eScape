...
export default function PointMapPanel({ title, points, selectedId, onSelect, onChooseLocation, routePoints = [], routeSegments = [], routeSummary = "", legend, label }) {
  ...
  return (
    <section className="point-map-panel glass-panel" aria-labelledby="point-map-heading">
      ...
      <PointMap points={points} selectedId={selectedId} onSelect={onSelect} onChooseLocation={onChooseLocation} routePoints={routePoints} routeSegments={routeSegments} label={label} />
      ...
      <AccessibleMapModal open={expanded} title={title} onClose={() => setExpanded(false)} returnFocusRef={expandRef}>
        <PointMap points={points} selectedId={selectedId} onSelect={onSelect} onChooseLocation={onChooseLocation} routePoints={routePoints} routeSegments={routeSegments} label={`Expanded ${label}`} expanded />
        <p className="point-map-legend"><strong>Legend:</strong> {legend}</p>
      </AccessibleMapModal>
    </section>
  );
